"""Health metric business service.

Device-provider payload parsing lives in ``business_support.services``. This
module only handles provider-neutral metric writes, manual records, querying,
and shared business effects such as task completion and metric alerts.
"""
...
class HealthMetricService:
    ...
    def save_device_metric(
        ...
        This method contains the business behavior shared by all device
        providers: daily limits, step overwrite semantics, daily task completion,
        ...
        if not cls._can_store_today(patient_id, metric_type, measured_at):
            logger.info(
                "患者 %s 当天 %s 记录已达上限，丢弃本次设备数据。",
                patient_id,
                metric_type,
            )
            return None
    ...
    def _save_device_steps(
        ...
        if metric:
            if metric.value_main is not None and value_main < metric.value_main:
                logger.info(
