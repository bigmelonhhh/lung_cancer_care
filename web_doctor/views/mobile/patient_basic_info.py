...
def api_mobile_patient_profile(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        patient_info = {
            ...
            "phone": patient.phone,
            "age": patient.age if getattr(patient, "age", None) is not None else "",
            "birth_date": str(patient.birth_date) if getattr(patient, "birth_date", None) else "请填写出生日期",
            "marital_status": getattr(patient, "marital_status", "") or "",
            "ethnicity": getattr(patient, "ethnicity", "") or "",
            "native_place": getattr(patient, "native_place", "") or "",
